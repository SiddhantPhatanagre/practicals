...
public class WorstFit {
    ...
    public static void main(String[] args) {
        ...
        // Execute the Worst Fit allocation algorithm
        worstFit();
    }

    // Worst Fit Allocation Algorithm
    static void worstFit() {
        // Iterate through each job to allocate it to the worst-fitting block
        for (int i = 0; i < numJobs; i++) {
            int worstIdx = -1; // Initialize with -1, meaning no block found

            // Search for the largest block that can fit the current job
            for (int j = 0; j < numBlocks; j++) {
                if (blockSizes[j] >= jobSizes[i]) {
                    // If no block has been selected, or if this block is a better fit, update worstIdx
            ...
            if (worstIdx != -1) {
                allocation[i] = worstIdx;             // Record the allocated block index
                blockSizes[worstIdx] -= jobSizes[i];  // Reduce the block size after allocation
            }
        }
